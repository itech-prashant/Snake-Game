...
int Temp_Body, Head, Tail;    // Temp_Body is the temp variable for snake body ; head and tail pointer of snake
...
void snakeInitialization()
{
    ...
    Temp_Body = y;    //Temp_Body is the temporary variable for printing the body of the snake
    Head = 5;
    Tail = 1;
    Game = 0;
    Frogs = 0; //initially number of frogs will be 0
    dir = 'd';
    score = 0, Speed = 99; //initial speed will be 99 an score will be 0

    for (i = 0; i < Head; i++)
    {
        Temp_Body++;
        Field[x][Temp_Body - Head] = i + 1; //setting the values for the body of the snake
    }
}
...
void ResetScreenPosition()
{
    ...
    HANDLE hOut; // hOut is a handle type variable, that provides the handle to the console of our program
    //(handle-Is sort of like a reference. Usually an integer or pointer value that refers to some object, )
    ...
    Position.X = 0;
    Position.Y = 0;
    //setting the Console cursor position with the help of hOut to position coordinates
    SetConsoleCursorPosition(hOut, Position);
}

void Random()
{
    srand(time(0));      //creating the complete random values with help of srand function using time
    ...
    if (Frogs == 0 && Field[a][b] == 0)
    {
        ...
        Field[a][b] = -1; // setting the field index to -1 to represent that the location is occupied now
        Frogs = 1;
...
void movement()
{
    ...
    if (dir == 'd')
    {
        ...
        if (y == M - 1)
            y = 0; //when the snake reaches the border it should come out from other side
        ...
        if (Field[x][y] == -1) //when snake eats the foot
        {
            ...
            score += 5; // increasing the score by 5
            Tail -= 1;  // increasing the length of the snake by decreasing the tail length
        }
        Head++;             //incrementing the head
...
void TailRemove()
{
    for (i = 0; i < N; i++)
    {
        for (j = 0; j < M; j++)
        {
            ...
            if (Field[i][j] == Tail)
            {
                // if the field value match then we will set that field value to 0
                Field[i][j] = 0;
            }
        }
    }
...
void GameOver()
{
    ...
    printf("\n\t\t\t\t!! GAME OVER !!\n\n");
    printf("\t\t\t\t YOUR SCORE : %d \n\n", score);
    printf("\t\t\tThanks for Playing Snake Game by Prashant \n\t\t\tPress ENTER to play again or ESC to exit ... \n");
    ...
    while (1)
    {
        ...
        {
            Game = 1; // when esc key is pressed
            break;
        }
    }
    system("Cls");
...
void main()
{
    do
    {
        ...
        printf("\n");
        printf("\t\t\t\t%c                Snake Game by Prashant                    %c\n", 186, 186);
        ...
        printf("\n");
        printf("\t\t\t\t%c         Press any key to play or 'Esc' to quit...        %c\n", 186, 186);
        for (i = 0; i < 60; i++)
        {
            if (i == 0)
            {
